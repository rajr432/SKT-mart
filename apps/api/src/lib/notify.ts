...
function renderEmail(title: string, body: string, link?: string) {
  ...
  const cta = link
    ? `<a href="${web}${link}" style="display:inline-block;margin-top:16px;padding:12px 22px;background:linear-gradient(90deg,#2874f0,#7b4bff);color:#fff;text-decoration:none;border-radius:8px;font-weight:600">View details →</a>`
    : "";
  ...
  <div style="max-width:560px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,.08)">
    ...
    <div style="padding:24px">
      ...
      <p style="color:#444;line-height:1.55;margin:8px 0 0;font-size:14px">${body}</p>
      ${cta}
    </div>
